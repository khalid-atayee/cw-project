     ...
     <div class="program-hero-section m-3 rounded-3">
         <div class="row ">
             ...
             <div class=" col-lg-7 col-sm-12 p-5 text-center text-md-start ">
                 ...
                 <p class="some-paragraph-title textAlign-left"> Lorem ipsum dolor sit amet, consectetur adipisicing
                     elit.
                 ...
                 <div class="course-info rounded-3 text-start mb-auto px-4 py-3">
                     ...
                     <h4 class="my-2 what-make-different-title">Codeweekend<span class="fw-bold">
                             @if (isset($data))
                                 {{ $data[0]->title . '-' . $data[0]->city->city_name }}
                             @else
                                 (Location)
                             @endif
                         </span>program</h4>
                     <h5> <span class="fw-bold landing-paragraph">Start Date:</span>
                         @if (isset($data))
                             {{ $data[0]->start_date }}
                     </h5>
                 @else
                     ...
                     <h5> <span class="fw-bold landing-paragraph">Duration:</span>
                         @if (isset($data))
                             {{ $data[0]->duration . ' Month' }}
                     </h5>
                 @else
                     <span>Not defined yet</span>
                     @endif
                     <h5> <span class="fw-bold landing-paragraph">Fees:</span>
                         @if (isset($data))
                             {{ '$ '. $data[0]->fees }}
                     </h5>
                 @else
                     ...
                     <p>
                         ...
                         <a href="{{ Auth::user() ? route('payment.index') : route('students.index') }}"
                             class="cw-btn core-btn">Apply Now</a>
                     </p>
                 </div>
